...
def corregir_solicitantes_vectorizado(df, lista_maestra_dict, columna):
    ...
    best_match_names = [match[0] if match and match[1] > 80 else solic for solic, match in zip(df[columna], best_matches)]
...
def create_ZMM621_COMODIN_OC_HES_HEM(df_ZMM621_fechaAprobacion):
    ...
    df_ZMM621_HES_HEM = sort_and_remove_duplicates(df_ZMM621_fechaAprobacion, ['COMODIN OC', 'Fecha de registro'], 'COMODIN OC')

    # Utilizamos una operación vectorizada para crear la columna 'Estado HES/HEM'
    mask = pd.isna(df_ZMM621_HES_HEM['Fecha de registro'])
...
class CoincidenciaBuscadorFinal:
    ...
    def buscar_coincidencia(self, columna_a_buscar, columna_busqueda, valor_coincidencia, 
       ...
       """
        ...
        if columna_a_buscar not in self.dataset_entrada.columns:
            print(f"Error: La columna '{columna_a_buscar}' no se encuentra en dataset_entrada.")
            return
...
def inmovilizadosConverted(df_inmovilizados, df_criticos):
    ...
    # Usar en caso Logistica no pase bien su archivo
    """
    ...
    desired_columns = ['Material', 'Descripción', 'Val.stk.valor', 'Moneda', 'Stock valorado', 'AREA', 'PEDIDO POR', 
                       'RESPONSABLE', 'UM', 'Últ.entr.', ' Últ.mov.', 'Tipo de Repuesto']
    ...
    df_inmovilizados.loc[mask_no_critico, "Deducción"] = df_inmovilizados.loc[mask_no_critico, "Dias Inmovilizados"].apply(get_tasa) * df_inmovilizados.loc[mask_no_critico, "Val.stk.valor"].astype(float)
    df_inmovilizados["SALDO"] = df_inmovilizados["Val.stk.valor"].astype(float) - df_inmovilizados["Deducción"]
